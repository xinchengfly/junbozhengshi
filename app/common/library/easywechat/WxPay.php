...
class WxPay
{
     ...
     * 统一下单API
     */
    public function unifiedorder($order_no, $openid, $totalFee, $orderType = OrderTypeEnum::MASTER, $pay_source, $multiple, $app_id)
    {
        $data = [
        ...
        //如果是微信小程序
        if ($pay_source == 'wx' || $pay_source == 'android' || $pay_source == 'ios') {
            $time = time();
            if ($pay_source == 'wx') {
                // 二次签名的参数必须与下面相同
                $params = [
    ...
    private function isServicePay($app_id)
    {
        ...
        if ($config['weixin_service']['is_open'] == 1 && $app['weixin_service'] == 1) {
            $this->app->config['app_id'] = $config['weixin_service']['app_id'];
            $this->app->config['mch_id'] = $config['weixin_service']['mch_id'];
            $this->app->config['key'] = $config['weixin_service']['apikey'];
            $filePath = root_path() . 'runtime/cert/appwx/10000/';
            $this->app->config['cert_path'] = $filePath . 'cert.pem';
            $this->app->config['key_path'] = $filePath . 'key.pem';
            $this->app->setSubMerchant($app_wx['mchid'], $app_wx['wxapp_id']);
            return true;
        } else {
            return false;
        }
    ...
    public function refund($transaction_id, $total_fee, $refund_fee)
    {
        // 当前时间
        $time = time();
        $result = $this->app->refund->byTransactionId($transaction_id, $time, $total_fee * 100, $refund_fee * 100, [
