...
class Product extends ProductModel
{
    ...
    {
        // 获取商品列表
        $data = (new ProductModel())->getList($param, 'api');
     ...
     * 根据商品id集获取商品列表
     */
    public function getListByIdsFromApi($productIds, $userInfo = false, $appid = '')
    {
        // 获取商品列表
        $data = (new ProductModel())->getListByIds($productIds, 10, $appid);
        // 整理列表数据并返回
        return $this->setProductListDataFromApi($data, true, ['userInfo' => $userInfo]);
    }
    ...
    public function getRecommendProduct($params)
    {
        ...
        if (!empty($params['appid'])){
            $applet_id = ApplistModel::detail(['appid'=>$params['appid']])['id'];
